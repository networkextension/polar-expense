// Build the expense plugin UI.
//
//   src/expense.ts  -> dist/scripts/expense.js   (esbuild bundle, ESM)
...
await esbuild.build({
    entryPoints: [path.join(srcDir, "expense.ts")],
    bundle: true,
    format: "esm",
    target: ["es2022"],
    platform: "browser",
...
// Copy public/ assets verbatim — expense.html and anything else the
...
console.log("built expense plugin UI -> dist/");
